...
export function AppPagination({ current = 1, total = 3, basePath = "/products" }: { current?: number; total?: number; basePath?: string }) {
  return (
    <nav className="mt-12 flex items-center justify-center gap-1" aria-label="Ph?n trang">
      <Link href={`${basePath}?page=${Math.max(1, current - 1)}`} aria-label="Trang tr??c" aria-disabled={current === 1} className={cn(buttonVariants({ variant: "outline", size: "icon-lg" }), current === 1 && "pointer-events-none opacity-50")}><ChevronLeft aria-hidden="true" /></Link>
