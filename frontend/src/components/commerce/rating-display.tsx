...
export function RatingDisplay({ rating, count, compact = false, className }: { rating: number; count?: number; compact?: boolean; className?: string }) {
  return (
    <div className={cn("flex items-center gap-1.5 text-sm", className)} aria-label={`${rating} tr?n 5 sao${count ? `, ${count} ??nh gi?` : ""}`}>
      <Star className="size-4 fill-warning text-warning" aria-hidden="true" />
      <span className="font-medium">{rating.toFixed(1)}</span>
      {!compact && count !== undefined ? <span className="text-muted-foreground">({count} ??nh gi?)</span> : null}
    </div>
  );
}
